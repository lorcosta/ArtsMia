...
public class ArtsmiaDAO {
	...
	public int getPeso(ArtObject a1, ArtObject a2) {
		String sql="SELECT COUNT(*) AS apparizioni " + 
		...
		try {
			...
			ResultSet res = st.executeQuery();
			if(res.next()){
				int peso=res.getInt("apparizioni");
				conn.close();
				return peso;
			}
			conn.close();
	...
	public List<Adiacenza> getAdiacenze(){
		String sql="SELECT eo1.object_id as obj1 ,eo2.object_id as obj2, COUNT(*) AS apparizioni " + 
				"FROM exhibition_objects AS eo1, exhibition_objects AS eo2 " + 
		...
		try {
			...
			ResultSet res = st.executeQuery();
			while(res.next()){
				adiacenze.add(new Adiacenza(res.getInt("obj1"),res.getInt("obj2"), res.getInt("apparizioni")));
			}
			conn.close();
			return adiacenze;
		} catch (SQLException e) {
